...
class CustomerSupportEnv:
    ...
    def reset(self, scenario_id: Optional[str] = None) -> Observation:
        ...
        self._obs = Observation(
            ...
            conversation_history=[f"[Customer] {first_turn.user_message}"],
    ...
    def step(self, action: Action) -> Tuple[Observation, float, bool, Dict[str, Any]]:
        ...
        agent_line = f"[Agent] {action.to_log()}"
        ...
        if not done and next_turn_idx < len(scenario.turns):
            ...
            conversation_history.append(f"[Customer] {next_user_msg}")
        else:
            next_user_msg = obs.user_message  # keep last message when done

        scalar_reward = reward_breakdown.total
